                    ...
                    <tbody>
                        @foreach($modelPrO as $pro)
                            <tr>
                                ...
                                <td class="tdEllipsis p-l-10">{{ $pro->wbs->code }} - {{ $pro->wbs->number }}</td>
                                <td class="p-l-10">{{ $pro->user->name }}</td>
                                @if($pro->status == 2)
                                    <td class="p-l-10">{{ 'RELEASED' }}</td>
                                @else
                                    <td class="p-l-10">{{ '' }}</td>
                                @endif
                                <td class="textCenter">
                                    @if($route == "/production_order")
                                        <a class="btn btn-primary btn-xs" href="{{ route('production_order.confirm', ['id'=>$pro->id]) }}">
                                            SELECT
                                        </a>
                                    @elseif($route == "/production_order_repair")
                                        <a class="btn btn-primary btn-xs" href="{{ route('production_order_repair.confirm', ['id'=>$pro->id]) }}">
                                            SELECT
                                        </a>
                                    @endif
                                </td>

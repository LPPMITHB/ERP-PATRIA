...
<li class="treeview {{ $menu->id == $menuInfo['parent2'] ? 'active' : $menu->id == $menuInfo['parent1'] ? 'active' : '' }}" data-toggle="tooltip" title="{{ $menu->name }}" data-container="body" data-placement="right">
    <a href="#" >
    ...
    <ul class="treeview-menu">
        @foreach($menu->menus as $menu2)
            ...
            <li class="treeview {{ $menu2->id == $menuInfo['parent1'] ? 'active' : '' }} " data-toggle="tooltip" title="{{ $menu2->name }}" data-container="body" data-placement="right">
                ...
                <ul class="treeview-menu">
                    @foreach($menu2->menus as $menu3)                    
                        <li class="{{ $menu3->id == $menuInfo['child'] ? 'active' : '' }}" data-toggle="tooltip" title="{{ $menu3->name }}" data-container="body" data-placement="right"><a href="{{ route($menu3->route_name)}}"><i class="fa {{ $menu3->icon }}" ></i>
                        @if(strlen($menu3->name) > 15)
                            <span>{{ substr($menu3->name, 0, 10) }}...</span>
                        @else

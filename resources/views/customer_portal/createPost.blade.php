...
@section('content-header')
    @breadcrumb(
        [
            'title' => 'Posts Complaints » '.$project->name,
            'items' => [
            ...
            <table id="posts-table" class="table table-hover table-striped tableFixed">
              ...
              <tbody>
                <tr v-for="(data, index) in posts">
                  <td style="width: 60%" class="tdEllipsis" class="mailbox-subject"><b>{{data.subject}}</b> - {{data.body}}
                  </td>
                  <td style="width: 10%"class="mailbox-attachment text-center" v-if="data.file_name != null"><i class="fa fa-paperclip"></i></td>
    ...
    <div v-show="create_post" class="col-md-12">
      <div class="box box-primary">
        ...
        <div class="box-body">
          ...
          <div class="form-group">
            <textarea id="compose-textarea" v-model="new_post.body" class="form-control" style="height: 300px;" placeholder="Body:"></textarea>
          </div>
          <div class="form-group">
        ...
        <div class="box-footer">
          <div class="pull-right">
            <button :disabled="createOk" @click.prevent="submitPost()" class="btn btn-primary"><i class="fa fa-envelope-o"></i> CREATE</button>
          </div>
        </div>
        <!-- /.box-footer -->
      </div>
    ...
    getPosts(){
      window.axios.get('/api/getPosts/'+this.project.id).then(({ data }) => {
        ...
        this.posts.forEach(post => {
          post.time_since = moment(post.created_at, "YYYY-MM-DD hh:mm:ss").fromNow();
        });

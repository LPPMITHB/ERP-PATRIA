...
class GoodsIssueController extends Controller
{
    ...
    public function updateSlocDetail($material_id, $sloc_detail, $issued)
    {
        ...
        if ($modelSlocDetail) {
            ...
            $modelMaterial = Material::find($sloc_detail->material_id);
            $materialSPC = json_decode($modelMaterial->cost_standard_price_config);
            $materialID = $sloc_detail->material_id;
            if ($materialSPC->id == 3) {
                $modelSLD = StorageLocationDetail::where('material_id', $sloc_detail->material_id)->get();
                $purchaseOderID = [];
                foreach ($modelSLD as $modelSLDS) {
                    $modelGR = GoodsReceipt::with([
                        'goodsReceiptDetails' =>
                        function ($query) use ($materialID) {
                            $query->where('material_id', $materialID)->get();
                        }
                    ]);
                    foreach ($modelGR as $modelGRS) {
                        array_push($purchaseOderID, $modelGRS->purchase_order_id);
                    }
                }
                $modelPO = PurchaseOrder::whereIn('id', $purchaseOderID)->orderBy('approval_date', 'ASC')->first();
                $modelPOD = PurchaseOrderDetail::where(['purchase_order_id' => $modelPO->id, 'material_id' => $sloc_detail->material_id])->first();
                $modelMaterial->cost_standard_price = $modelPOD->total_price / $modelPOD->quantity;
